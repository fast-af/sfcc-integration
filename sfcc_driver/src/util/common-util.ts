...
const jsonReader = () => {
  try {
    const filePath = path.join(__dirname, "..", `/${process.env.PLATFORM_FLAG || "sfcc"}/util/messages.json`);
...
const getMessage = ((errCode: string, action: ActionType): string => {
  return `${action}: ${(process.env.PLATFORM_FLAG || "sfcc").toUpperCase()} API: ${Messages[errCode]}`;
});
